...
import { meetsched, vmeeting } from '../../meetingschedule/meeting';
import { MeetingupdateService } from '../Service/meetingupdate.service';
declare var $: any;


@Component({
  selector: 'app-meetingupdatelist',
  templateUrl: './meetingupdatelist.component.html',
  styleUrls: ['./meetingupdatelist.component.scss']
})
export class MeetingupdatelistComponent implements OnInit {
	meetupdateinfo$: Observable<Root<vmeeting[]>> = of();
	...
	ngOnInit(): void {
		this.loadmeetupdate().subscribe(() => {
			this.initializeDataTable();
		});
	}

	loadmeetupdate(): Observable<Root<vmeeting[]>> {
		return this.meetupdateservices.getmeetupdate().pipe(
			tap((data) => {
				if (data.isSuccess) {
					return this.meetupdateinfo$ = of(data);
				} else {
  ...
  navigate() {
	...
	printData() {
		this.meetupdateservices.generateReports().subscribe({
	...
	navigatenew(items: vmeeting){
		this.router.navigateByUrl('/dashboard/meetingupdate', { state: items });
	}	

  navigateres(items: vmeeting){
		this.router.navigateByUrl('/dashboard/meetingupdate', { state: items });
	}	
}
