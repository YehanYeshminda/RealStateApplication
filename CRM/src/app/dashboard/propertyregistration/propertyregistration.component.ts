...
@Component({
  selector: 'app-propertyregistration',
  templateUrl: './propertyregistration.component.html',
  styleUrls: ['./propertyregistration.component.scss']
})
export class PropertyregistrationComponent implements OnInit {
  ...
  otherImages: Array<{ url: string }> = []; // Array to store uploaded main images
  ...
  ngOnInit(): void {
    ...
    // this.userImage = this.baseUrl + 'upload/property/main/' + "h.png.png";
  }
  ...
  NavigateCatto(isPropcat: boolean) {
    const initialState: ModalOptions = {
      initialState: {
        isPropcat: isPropcat
      },
  ...
  NavigateSubto(isPropsubcat: boolean) {
    const initialState: ModalOptions = {
      initialState: {
        isPropsubcat: isPropsubcat
      },
      class: 'modal-lg',
      backdrop: 'static',
    };
