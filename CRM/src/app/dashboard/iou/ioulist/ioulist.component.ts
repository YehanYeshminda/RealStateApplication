...
@Component({
  selector: 'app-ioulist',
  templateUrl: './ioulist.component.html',
  styleUrls: ['./ioulist.component.scss']
})
export class IoulistComponent implements OnInit {
	iouinfo$: Observable<Root<viou[]>> = of();
	...
	ngOnInit(): void {
		this.loadiou().subscribe(() => {
			this.initializeDataTable();
		});
	}

	loadiou(): Observable<Root<viou[]>> {
		return this.iouservices.getiou().pipe(
			tap((data) => {
				if (data.isSuccess) {
					return this.iouinfo$ = of(data);
				} else {
  ...
  navigate() {
		this.router.navigateByUrl('/dashboard/iou');
	}
	...
	printData() {
		this.iouservices.generateReports().subscribe({
	...
	navigatenew(items: viou){
		this.router.navigateByUrl('/dashboard/iou', { state: items });
	}

	  

	printCellData(id: number) {
		this.iouservices.cellreport(id).subscribe({
		  next: (response: HtmlResponse) => {
			if (response) {
			  const printWindow = window.open('', '');
